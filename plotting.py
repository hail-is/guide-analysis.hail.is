...
def var_contrib_to_lat(
    ...
    for i, lat in enumerate(cols):
        ...
        leg = axes[i].legend(
            handles[::-1], labels[::-1], loc='upper left', bbox_to_anchor=(1.05, 1.2), fontsize='6'
        )
        print('LABELS')
        print(labels)
...
def pheno_contribs_to_lat(df, cols, topn=3, spacing=2.3, figname=None):
    ...
    if np.any(other_contrib > 0):
        ax.bar(ind, other_contrib, width, bottom=cumulative_vec, label='Other', color='lightgrey')
        phenos_seen.append('Other')
        other_plotted = True

    # 4. Handle Legend
    leg = ax.legend(
        loc='upper left', bbox_to_anchor=(1.02, 1), borderaxespad=0.0, fontsize=8, frameon=False
    )
