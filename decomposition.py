...
class decomposition:
    ...
    def get_topk_pcs_for_phe_with_scores_by_label(self, label, topk=None):
        return self.get_topk_pcs_for_phe_with_scores_by_index(self.get_phe_index(label), topk=topk)
    ...
    def get_contribution_score_by_label_generic(self, target, pc_0index, label):
        return self.d['contribution_{}'.format(target)][
            self.get_index_generic(target, label), pc_0index
        ]
    ...
    def plot_data_contribution_generic(
        ...
        sort=True,
        append_others=False,
        contribution_thr=None,
        contribution_cumsum_thr=None,
    ):
        ...
        if contribution_thr is not None:
            topk = np.sum(self.d[contribution_key][:, pc_index] >= contribution_thr)
    ...
    def plot_data_contribution_phe(
        ...
        return self.plot_data_contribution_generic(
            'phe', pc_index, topk, sort, append_others, contribution_thr, contribution_cumsum_thr
        )
    ...
    def plot_data_contribution_var(
        ...
        return self.plot_data_contribution_generic(
            'var', pc_index, topk, sort, append_others, contribution_thr, contribution_cumsum_thr
        )
    ...
    def plot_data_contribution_gene(
        ...
        return self.plot_data_contribution_generic(
            'gene', pc_index, topk, sort, append_others, contribution_thr, contribution_cumsum_thr
        )
    ...
    def text_data_contribution_gene(
        ...
        query_str = '\n'.join(['{},{:.4f}'.format(x[0], x[1]) for x in zip(labels, scores)])
    ...
    def text_contribution_generic(
        ...
        plot_d = self.plot_data_contribution_generic(
            target, pc_index, topk, sort, append_others, contribution_thr, contribution_cumsum_thr
        )
        return delim.join(
            [
                string_format.format(l, y * value_multiplier)
                for l, y in zip(plot_d[labels], plot_d[values])
            ]
        )
    ...
    def text_description_phe_by_index(
        ...
        pcs, squared_cos_scores = self.get_topk_pcs_for_phe_with_scores_by_index(
            index, topk=num_of_top_pcs
        )
        return 'Top {} components for {} is: '.format(
            ...
            [
                ...
                for pc_index, squared_cos_score in zip(pcs, squared_cos_scores)
            ]
        )
