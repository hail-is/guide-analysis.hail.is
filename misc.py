...
def color_pallet_with_gray(n):
    ...
    return np.vstack([color_pallet(n - 1), np.array([0, 0, 0, 0.5])])
...
def build_from_guide(degas_npz_file_path, w_xl, w_lt, out_prefix=None):
    ...
    np.savez(
        ...
        contribution_phe=contribution_phe,
        factor_var=beta_l,
        factor_phe=lambda_l,
        label_var=degas_all['label_var'],
        label_phe=degas_all['label_phe'],
        cos_var=cos_var,
        cos_phe=cos_phe,
        label_phe_code=degas_all['label_phe_code'],
        label_gene=degas_all['label_gene'],
        label_phe_stackedbar=degas_all['label_phe_stackedbar'],
        contribution_gene=degas_all['contribution_gene'],
    )

    return out_path


def compute_contribution_gene(label_var, contribution_var, var_gene_file):
...
def compute_contribution_cyto(label_var, contribution_var, var_gene_file):
    # variant_df = pd.read_csv(variant_tsv, sep='\t', compression='gzip')
    # var2gene_dict = dict(zip(variant_df['label_var'], variant_df['label_gene']))
    # var2gene_dict = dict()
    var2cyto_dict = dict()
