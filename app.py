...
try:
    ...
    w_values_available = True
    logw_mat_TL = w_values_data['logw_mat_TL']
    logw_mat_XL = w_values_data['logw_mat_XL']
    print("w-values loaded successfully!")
except:
    w_values_available = False
    logw_mat_TL = None
    logw_mat_XL = None
    print("w-values not found. Run compute_w_values.py first.")
try:
    zscores = np.load('./degas_betas.npy')
    pvalues = 2 * (1 - norm.cdf(np.abs(zscores)))
    log10pvalues = -np.log10(np.clip(pvalues, a_min=1e-300, a_max=None))
    log10pvalues = np.clip(log10pvalues, a_min=0, a_max=None)
    print(f"Z-scores loaded successfully! Shape: {zscores.shape}")
    print(f"-log10(p) range: [{log10pvalues.min():.2f}, {log10pvalues.max():.2f}]")
    zscores_available = True
except Exception as e:
    print(f"Could not load z-scores: {e}")
    zscores = None
    pvalues = None
    log10pvalues = None
    zscores_available = False
def color_pallet(n):
    if(n > 12):
...
def compute_gene_contrib_to_cyto(label_var, contribution_var, var_gene_file):
    ...
    contribution_var_df['gene'] = [var2gene_dict.get(x, '') for x in label_var]
...
def parse_variant_label(var_label):
    try:
        ...
        return None, None
def prepare_manhattan_data(var_labels, contrib_var, trait_name, contrib_phe, var_gene_file='./snp_gene_pq.txt'):
    chr_list = []
    pos_list = []
    ...
    trait_idx = np.where(gz['label_phe'] == trait_name)[0]
    if len(trait_idx) == 0:
        return None
    ...
    return df
def plot_manhattan(data_df, value_col='value', title="Manhattan Plots", 
                   figsize=(16, 6), 
                   chr_filter=None, region_start=None, region_end=None,
                   use_pvalues=True):
    plt.clf()
    plt.close('all')
    ...
    if chr_filter is not None:
        ...
        if region_start is not None and region_end is not None:
            data_df = data_df[(data_df['POS'] >= region_start) & 
                              (data_df['POS'] <= region_end)].copy()
    data_df = data_df.sort_values(['CHR', 'POS']).copy()
    if len(data_df) == 0:
        ax.text(0.5, 0.5, 'No data in selected region', 
                ha='center', va='center', transform=ax.transAxes, fontsize=14)
        return fig, ax
    print(f"DEBUG: value column min={data_df[value_col].min():.4f}, max={data_df[value_col].max():.4f}")
    ...
    top3_latent_labels = data_df.attrs.get('top3_latent_labels', ['1st Latent', '2nd Latent', '3rd Latent'])
    if use_pvalues:
    ...
    if chr_filter is None:
        ...
        ax.set_xlabel(f'Position on Chromosome {chr_filter} (bp)', 
    ...
    if highlight_threshold <= y_max + y_padding:
        ax.axhline(y=highlight_threshold, color='red', linestyle='--', 
    ...
    if len(handles) >= 4:
        ...
        ax.legend(ordered_handles, ordered_labels, 
                  loc='upper left', bbox_to_anchor=(1.0, 1), frameon=True, 
                  fancybox=True, shadow=True, fontsize=9)
    else:
        ax.legend(loc='upper left', bbox_to_anchor=(1.0, 1), frameon=True, 
                  fancybox=True, shadow=True, fontsize=9)
...
def create_enhanced_phenotype_table(gz, w_values_available, logw_mat_TL):
    pheno_labels = gz['label_phe']
    contrib_phe = gz['contribution_phe']
...
enhanced_pheno_table = create_enhanced_phenotype_table(gz, w_values_available, logw_mat_TL)
plots = ui.page_sidebar(
    ui.sidebar(
        ui.input_selectize(
            "pheno_select", "Choose a phenotype:",
            list(gz['label_phe']),
            selected="Alzheimer's disease/dementia"
        ),
        ui.input_slider("n_var_components", "Number of Latent Factors", 
                        min=1, max=10, value=5, step=1.0),
        ui.input_slider("n_phenotypes_show", "Number of Phenotypes in Plot", 
                        min=1, max=5, value=3, step=1.0),
        ui.input_slider("topn_loci", "Number of Loci in Plot", 
                        min=1, max=15, value=9, step=1.0),
        ui.input_slider("topn_genes", "Number of Example Genes per Locus", 
                        min=1, max=5, value=2, step=1.0),
    ...
    ui.row(
        ...
        ui.column(9,
            ui.output_plot("pheno_contrib_to_lat_controller")
        ),
    ),
    ui.row(
        ui.output_plot("var_contrib_to_lat_controller", height=f"{PLOT_HEIGHT}px")
    )
)
table = ui.page_fluid(
    ui.HTML(
...
def server(input: Inputs, output: Outputs, session: Session):
    ...
    def plot_variance_components_controller():
        ...
        paths = [f'./all_phenos/phenotypes/guide/{get_safe_phe_name(p)}/squared_cosine_scores.tsv' 
    ...
    def var_contrib_to_lat_controller():
        if not latent_labels:
            ...
            ax.text(0.5, 0.5, 'Please select a phenotype first', 
                    ha='center', va='center', transform=ax.transAxes)
            ax.axis('off')
            return fig
        interested_phenos = input.pheno_select()
    ...
    def pheno_contrib_to_lat_controller():
        ...
        fig = pheno_contribs_to_lat(
            df, 
            latent_labels, 
            topn=input.n_phenotypes_show(),
            spacing=3
        )
        return fig
    @session.download(
    ...
    async def download_genes():
        if not latent_labels:
            ...
            return
        yield f"Gene_Name\tVariance_Component\n"
    ...
    def filtered_table():
        ...
        if latent_filter:
            mask = df['latent_info'].apply(
                lambda info_list: any(latent_filter.lower() in item['latent'].lower() for item in info_list)
            )
            df = df[mask]
    ...
    def custom_table():
        df = filtered_table()
        if len(df) == 0:
            return ui.HTML("<p style='text-align: center; padding: 40px; color: #999;'>No phenotypes match your filters.</p>")
        html_parts = ["""
        <style>
            .pheno-table {
                width: 100%;
    ...
    @session.download(
        filename=lambda: f"guide-phenotype-table-{date.today().isoformat()}.csv"
    )
    ...
    def manhattan_plot():
        if not input.manhattan_trait():
            ...
            ax.text(0.5, 0.5, 'Please select a trait', 
                    ha='center', va='center', transform=ax.transAxes, fontsize=14)
            ax.axis('off')
            return fig
        chr_filter = None
        ...
        if input.manhattan_regional():
            ...
            if region_start >= region_end:
                fig, ax = plt.subplots(figsize=(16, 6))
                ax.text(0.5, 0.5, 'Error: Start position must be less than end position', 
                        ha='center', va='center', transform=ax.transAxes, 
        ...
        if df is None:
            fig, ax = plt.subplots(figsize=(16, 6))
            ax.text(0.5, 0.5, 'Error: Could not load data for trait', 
                    ha='center', va='center', transform=ax.transAxes, 
                    fontsize=14, color='red')
            ax.axis('off')
            return fig
    ...
    def manhattan_info_ui():
        ...
        for i, (lat_label, contrib, w_val) in enumerate(zip(top3_latent_labels, top3_contributions, top3_w_values), 1):
            if not pd.isna(w_val):
                info_parts.append(f"{i}. **{lat_label}** (variance component: {contrib:.4f}, -log10(w-value): {w_val:.4f})")
            else:
