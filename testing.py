...
def gen_data(var_labels,
             contrib_var_guide,
             var_gene_file='./snp_gene_pq.txt'):
    
    cyto_c_guide, names_guide, gene2cyto_dict_guide = compute_gene_contrib_to_cyto(var_labels, contrib_var_guide, var_gene_file)
...
cyto_contrib_guide, gene2cyto_dict_guide = gen_data(var_labels, contrib_var_guide, var_gene_file='./snp_gene_pq.txt')
...
plots = ui.page_fluid(
    ...
    ui.layout_sidebar(
        ui.panel_sidebar(
            ...
            ui.input_slider("n_var_components", "Number of Latent Factors", min=1, max=10, value=5, step=1.0),
            ui.input_slider("n_phenotypes_show", "Number of Phenotypes in Plot", min=1, max=5, value=3, step=1.0),
            ui.input_slider("topn_loci", "Number of Loci in Plot", min=1, max=15, value=9, step=1.0),
            ui.input_slider("topn_genes", "Number of Example Genes per Locus", min=1, max=5, value=2, step=1.0),
            ui.input_slider("topn_genes", "Number of Example Genes per Locus", min=1, max=5, value=2, step=1.0),
        ...
        ui.panel_main(
            ...
            ui.row(
                ui.output_plot("var_contrib_to_lat_controller",
                               height=PLOT_HEIGHT,
                               fill='page_fillable')
            )
        ),
...
def server(input: Inputs, output: Outputs, session: Session):    
    ...
    def plot_variance_components_controller():
        ...
        paths = [f'./all_phenos/phenotypes/guide/{get_safe_phe_name(p)}/squared_cosine_scores.tsv' for p in interested_phenos]
        ...
        fig, ax, latent_labels = plot_variance_components(df, 
    ...
    def pheno_contrib_to_lat_controller():
        ...
        fig = pheno_contribs_to_lat(df, latent_labels, topn=input.n_phenotypes_show(), 
                                        spacing=3)
        return fig
